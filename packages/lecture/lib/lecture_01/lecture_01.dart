/* 
  ...
  Порядок роста:
    - 1 - константная (выбор из массива по индексу, хэш-таблицы)
    - log n - логарифмическая (такая зависимость появляется, если на каждом шаге итерации задача сокращается)
    - n - линейная (обработка данных перебором)
    - n log n - квазилинейная (большинство алгоритмов декомпозиции)
    - n ^ 2 - квадратичный (вложенный цикл)
    - n ^ k - полиномиальный (k вложенных циклов)
    - 2 ^ n - экспоненциальный (обработка всех подмножеств)
    - n! - факториал (перестановки)

  Эффективность процессора при выполнении действий:
    int a = 15;
    - result = a * 2 // Умножение чуть медленнее, чем сложение
    - result = a + a // Сложение чуть быстрее чем умножение
    - result = a << 1 // Самый быстрый способ
